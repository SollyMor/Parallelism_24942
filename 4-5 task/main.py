...
class Sensor:
    ...
    def get(self):
        """Вернуть следующее значение датчика."""
        raise NotImplementedError("Subclasses must implement method get()")


class SensorX(Sensor):
    """Имитация датчика с заданной задержкой."""

    def __init__(self, delay: float):
        """delay — пауза между чтениями в секундах."""
        self._delay = delay
        self._data = 0

    def get(self) -> int:
        """Увеличить счётчик и вернуть его."""
...
class SensorCam(Sensor):
    ...
    def _camera_index(camera_name: str) -> int | str:
        """Преобразовать имя камеры в индекс или путь."""
        return int(camera_name) if camera_name.isdigit() else camera_name


class WindowImage:
    """Окно отображения кадра."""
...
def setup_logging() -> None:
    """Настроить запись логов в log/task4.log."""
    log_dir = Path("log")
...
def parse_resolution(value: str) -> tuple[int, int]:
    """Разобрать строку вида 1280x720."""
    try:
...
def put_latest(out_queue: queue.Queue, value: Any) -> None:
    """Положить значение в очередь, отбросив старое при переполнении."""
    try:
...
def sensor_worker(name: str, sensor: Sensor, out_queue: queue.Queue, stop_event: threading.Event) -> None:
    """Читать датчик в цикле и публиковать последнее значение."""
...
def get_latest(in_queue: queue.Queue, previous: Any) -> Any:
    """Забрать самое свежее значение из очереди."""
    value = previous
    while True:
        try:
...
def make_image(frame, sensor_values: dict[str, Any], resolution: tuple[int, int]):
    """Собрать кадр с подписями значений датчиков."""
    width, height = resolution
    if frame is None:
...
def main() -> int:
    """Запустить потоки датчиков и цикл отображения."""
    setup_logging()
    args = parse_args()
